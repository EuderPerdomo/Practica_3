...
const listar_garantias_admin = async function(req,res){
    if(req.user){
        if(req.user.role=='admin'){
            ...
            let reg =await Garantia.find({modelo: new RegExp(filtro,'i')})
...
const agregar_observacion_garantia_admin=async function(req,res){
    if(req.user){
        if(req.user.role=='admin'){
            ...
            var reg =await Garantia.findByIdAndUpdate({_id:id},{ $push: {observaciones:{
                fecha:Date.now(),
        ...
        if(req.user.role=='admin'){
           ...
           idd= new mongoose.Types.ObjectId(data.id_bodega)
            ...
            var reg=await Bodega.updateOne(
                ...
                var rep_garantia =await Garantia.findByIdAndUpdate({_id:data.id_garantia},{ $push: {repuestos:{
                    fecha:Date.now(),
