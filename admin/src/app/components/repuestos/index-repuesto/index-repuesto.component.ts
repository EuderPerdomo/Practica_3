...
export class IndexRepuestoComponent implements OnInit {
 ...
 // public load = false;
  ...
  filtros(tipo: any) {
    if (tipo == "modelo") {
      if (this.filtro_modelo) {
        console.log(this.filtro_modelo)
        this.load_data = true
        this._repuestoService.listar_repuestos_admin(tipo, this.filtro_modelo, this.token).subscribe(
          response => {
...
/*
  iinit_data() {
    this.load = true;
    this._repuestoService.listar_repuestos_admin(null,null, this.token).subscribe(
      response => {
